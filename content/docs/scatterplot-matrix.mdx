...
The Scatterplot Matrix (also known as SPLOM) is a powerful visualization technique for exploring relationships between multiple numeric variables simultaneously. This implementation features interactive brushing that allows you to select data points across all plots in the matrix, making it easy to identify patterns and correlations in multi-dimensional datasets.
...
- **Color-coded categories** - Distinguish data by species, category, or any categorical dimension
...
- `d3-scale` - Linear scales for axes and color mapping
...
- One categorical column for color-coding
...
interface DataItem {
  ...
  // Categorical dimension for coloring
  category: string;
...
| `dimension` | `string` | Name of the categorical field to use for color-coding |
...
2. Selected points appear in **color**
3. Unselected points appear as small **gray/white dots**
4. Selection is reflected across **all cells** in the matrix
5. Click outside the brush or in another cell to clear selection

### Color Encoding

Each unique value in the `dimension` column gets a distinct color from the Observable10 color scheme:
...
3. **Compare categories**: Look for separation between colored groups
...
- Ensure color scale domain is being set correctly
...
- **Bubble Chart** - For 3-4 dimensions with size/color encoding
