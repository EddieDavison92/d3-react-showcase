...
- `d3-scale` - Linear and band scales for axes and color mapping
- `d3-axis` - Axis generators
- `d3-brush` - 2D and 1D brushing behaviors
- `d3-selection` - DOM manipulation and event handling
- `d3-scale-chromatic` - Color schemes (interpolateRdYlBu)
...
- **Color**: Temperature anomaly value

Each cell represents one month in one year, colored according to the temperature anomaly.
...
#### Legend Brush
1. Click and drag on the color legend to select a temperature range
...
The `interpolateRdYlBu` color scheme:
...
Choose alternative D3 color interpolators:
...
.html(`
  ...
  <div class="text-sm">
    Temperature: ${d.Anomaly?.toFixed(2)}°C
    <br/>
...
- Verify color scale domain is correct
...
- Verify color scale domain matches data
