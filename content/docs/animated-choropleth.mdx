...
description: Interactive geographic visualisation with animation and custom projections
---

## Overview

The Animated Choropleth Map is an interactive geographic visualisation that displays data across geographic regions using colour gradients. This implementation features smooth animations between data dimensions, interactive tooltips, clickable regions, and support for custom map projections.

## Features

- **Animated transitions** between different data dimensions
- **25+ colour schemes** from D3's colour interpolators
...
- **Colour legend** with dynamic range based on current data
...
- `d3-scale` - Colour scales for data mapping
...
| `colorScheme` | `(t: number) => string` | D3 colour interpolator function |
...
D3 provides 25+ colour schemes. Import from `d3-scale-chromatic`:
...
Create a colour scheme selector:
...
1. **Data optimisation**: Pre-filter and aggregate data before passing to component
...
- Ensure colour scheme is imported correctly
...
- `d3-colour-selector` - Colour scheme dropdown component

## Resources

- [D3 Geographic Projections](https://github.com/d3/d3-geo)
- [D3 Colour Schemes](https://github.com/d3/d3-scale-chromatic)
